...
- **Empirical (resolved by walking-skeleton `infrastructure-006`):** does `AudioHardwareCreateProcessTap` work inside the macOS App Sandbox? Currently no documentation says yes. The spike runs unsandboxed; sandbox compatibility is a follow-up question for the real-Process-Tap implementation.
